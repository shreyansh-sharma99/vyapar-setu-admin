...
import {
  ...
  FileText, ArrowRightCircle, Receipt, Clock, CheckCircle2, XCircle, AlertCircle, RefreshCw
} from 'lucide-react';
...
export default function QuotationList() {
  ...
  const { quotations, summary, loading, meta, toast: reduxToast } = useSelector((state) => state.quotation);
  ...
  useEffect(() => { fetchQuotations(); }, [dispatch, currentPage, pageSize, selectedStatus]);
  ...
  const handleViewClick = (item) => {
    ...
    navigate(`/sales/quotations/view/${encId}`);
  };

  const handleEditClick = (item) => {
    if (item.status !== 'draft') { showToast('Only draft quotations can be edited.', 'warning'); return; }
    const encId = encodeURIComponent(encryptData(item._id));
    navigate(`/sales/quotations/edit/${encId}`);
  };

  const handleDeleteClick = (item) => {
    if (item.status !== 'draft') { showToast('Only draft quotations can be deleted.', 'warning'); return; }
    setDeleteModal({ isOpen: true, id: item._id });
  };

  const handleConfirmDelete = () => {
    if (deleteModal.id) { dispatch(deleteQuotation(deleteModal.id)); }
  ...
  const handleConfirmAction = () => {
    ...
    if (type === 'send') dispatch(sendQuotation(id));
    else if (type === 'accept') dispatch(acceptQuotation(id));
    else if (type === 'reject') dispatch(rejectQuotation(id));
    else if (type === 'proforma') dispatch(convertToProforma(id));
    else if (type === 'invoice') dispatch(convertToInvoice(id));
    setConfirmModal({ isOpen: false, type: null, id: null });
  };

  const handleDuplicate = (id) => dispatch(duplicateQuotation(id));
  ...
  const headers = [
    ...
    },
    {
      ...
      sortable: true,
      value: 'checked',
      render: (item) => <span>{formatDateWithTiming(item.quotationDate)}</span>,
    },
    {
      label: 'Valid Until',
      key: 'validUntil',
  ...
  const confirmConfig = {
    send: { title: 'Send Quotation?', message: 'This will mark the quotation as sent to the customer.', label: 'Send', variant: 'primary', icon: <Send className="w-6 h-6 text-blue-500" /> },
    ...
    invoice: { title: 'Convert to Invoice?', message: 'A new sales invoice will be created from this quotation and it will be marked converted.', label: 'Convert', variant: 'primary', icon: <Receipt className="w-6 h-6 text-emerald-500" /> },
  };
    ...
    <div>
      ...
      >
        <div className="pb-2">
          <TableInfoCard
            stats={Object.entries(STATUS_CONFIG).map(([key, cfg]) => {
              ...
              return {
                label: cfg.label,
                value: metric.count || quotations.filter((q) => q.status === key).length,
        ...
        <Table
          ...
          onSearchClick={() => { setCurrentPage(1); fetchQuotations(); }}
          onSearchClear={handleClearSearch}
          actions={
            <div className="flex items-center gap-2">
              <Button onClick={() => navigate('/sales/quotations/create')}>New Quotation</Button>
            </div>
          }
      ...
      <DeleteModal
        ...
        message="Are you sure you want to delete this draft quotation? This cannot be undone."
      />
      ...
      {activeDropdownId && dropdownAnchor && dropdownItem && createPortal(
        <>
          ...
          >
            ...
            {/* Accept / Reject (sent) */}
            {dropdownItem.status === 'sent' && (
              <>
                <button onClick={() => { setActiveDropdownId(null); openConfirm('accept', dropdownItem._id); }}
                  ...
                  <Check className="w-4 h-4 mr-2.5" /> Mark Accepted
                </button>
                <button onClick={() => { setActiveDropdownId(null); openConfirm('reject', dropdownItem._id); }}
                  ...
                  <X className="w-4 h-4 mr-2.5" /> Mark Rejected
                </button>
              </>
            )}

            {/* Convert actions (accepted or sent) */}
            {(dropdownItem.status === 'accepted' || dropdownItem.status === 'sent') && (
              <>
                <div className="mx-3 my-1 border-t border-[var(--vs-border)]" />
                <button onClick={() => { setActiveDropdownId(null); openConfirm('proforma', dropdownItem._id); }}
                  className="flex items-center w-full px-4 py-2.5 text-left hover:bg-[var(--vs-drop-hover)] transition-colors cursor-pointer text-violet-600 dark:text-violet-400">
                  <FileText className="w-4 h-4 mr-2.5" /> → Proforma Invoice
                </button>
