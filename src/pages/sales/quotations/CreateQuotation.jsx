...
import { ArrowLeft, Plus, Trash2, Save, Send, Calendar, FileText } from 'lucide-react';
...
export default function CreateQuotation() {
  ...
  const { currentQuotation, loading: quotationLoading, success, error } = useSelector((s) => s.quotation);
  ...
  const [notes, setNotes] = useState('Thank you for your interest.');
  ...
  const [lineItems, setLineItems] = useState([
    { productId: '', productName: '', hsnCode: '', sku: '', qty: 1, unit: 'pcs', rate: 0, discountPercent: 0, isGstApplicable: true, taxPercent: 18, cessPercent: 0 },
  ]);
  ...
  useEffect(() => {
    ...
    dispatch(resetQuotationStatus());
    if (isEditMode) dispatch(getQuotationById(id));
    return () => { dispatch(clearCurrentQuotation()); dispatch(resetQuotationStatus()); };
  }, [dispatch, id, isEditMode]);

  useEffect(() => {
    if (isEditMode && currentQuotation) {
      setSelectedCustomerId(currentQuotation.customerId?._id || '');
      setCustomerSnapshot(currentQuotation.customerSnapshot || {
        name: '', phone: '', email: '', gstin: '', stateCode: '',
        billingAddress: { street: '', city: '', state: '', pincode: '', country: 'India' },
        shippingAddress: { street: '', city: '', state: '', pincode: '', country: 'India' },
      });
      setQuotationDate(currentQuotation.quotationDate ? new Date(currentQuotation.quotationDate).toISOString().split('T')[0] : '');
      setValidUntil(currentQuotation.validUntil ? new Date(currentQuotation.validUntil).toISOString().split('T')[0] : '');
      setPlaceOfSupply(currentQuotation.placeOfSupply || '');
      setInvoiceType(currentQuotation.invoiceType || 'B2C');
      setReverseCharge(!!currentQuotation.reverseCharge);
      setOverallDiscountPercent(currentQuotation.overallDiscountPercent || 0);
      setRoundOff(currentQuotation.roundOff !== false);
      setNotes(currentQuotation.notes || '');
      setTerms(currentQuotation.terms || '');
      if (currentQuotation.lineItems?.length > 0) {
        setLineItems(currentQuotation.lineItems.map((item) => ({
  ...
  }, [currentQuotation, isEditMode]);

  useEffect(() => {
    if (success) { dispatch(resetQuotationStatus()); navigate('/sales/quotations'); }
  ...
  const handleCustomerChange = (customerId) => {
    ...
    if (!customerId) {
      setCustomerSnapshot({ name: '', phone: '', email: '', gstin: '', stateCode: '', billingAddress: { street: '', city: '', state: '', pincode: '', country: 'India' }, shippingAddress: { street: '', city: '', state: '', pincode: '', country: 'India' } });
      setInvoiceType('B2C');
      return;
    }
    ...
    if (customer) {
      ...
      const snap = {
        ...
        billingAddress: { street: addr.street || '', city: addr.city || '', state: addr.state || '', pincode: addr.pincode || '', country: addr.country || 'India' },
        shippingAddress: { street: addr.street || '', city: addr.city || '', state: addr.state || '', pincode: addr.pincode || '', country: addr.country || 'India' },
      };
  ...
  const handlePincodeChange = async (e) => {
    ...
    setCustomerSnapshot((prev) => ({ ...prev, billingAddress: { ...prev.billingAddress, pincode: val } }));
    if (val.length === 6) {
      ...
      try {
        ...
        if (data?.[0]?.Status === 'Success') {
          ...
          setCustomerSnapshot((prev) => ({ ...prev, billingAddress: { ...prev.billingAddress, city: po.District, state: po.State } }));
        }
  ...
  const handleLineItemChange = (index, field, value) => {
    ...
    if (field === 'productId') {
      ...
      if (product) {
        ...
        updated[index] = { ...updated[index], productId: '', productName: '', sku: '', hsnCode: '', rate: 0 };
      }
    } else {
  ...
  const addLineItem = () => setLineItems([...lineItems, { productId: '', productName: '', hsnCode: '', sku: '', qty: 1, unit: 'pcs', rate: 0, discountPercent: 0, taxPercent: 18, cessPercent: 0 }]);
  ...
  const calculateTotals = () => {
    ...
    lineItems.forEach((item) => {
      ...
      const discPct = Number(item.discountPercent || 0), taxPct = Number(item.taxPercent || 0);
      const taxable = qty * rate * (1 - discPct / 100);
      subtotal += taxable;
      taxAmount += taxable * (taxPct / 100);
    });
    const overallDiscount = subtotal * (Number(overallDiscountPercent) / 100);
    let total = subtotal + taxAmount - overallDiscount;
    if (roundOff) total = Math.round(total);
    return { subtotal, taxAmount, overallDiscount, totalAmount: total };
  };
  ...
  const handleSave = (status = 'draft') => {
    ...
    if (isEditMode) dispatch(updateQuotation({ id, payload }));
    else dispatch(createQuotation(payload));
  };
  ...
  if (isEditMode && quotationLoading) {
    return (
      <Card h1="Quotation" buttonName="Back" navigation="-1" buttonVariant="danger" buttonIcon={<ArrowLeft className="w-3.5 h-3.5" />} bodyClassName="p-6">
        <Loader />
      </Card>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6 mx-auto text-[var(--vs-text-primary)]">
      <Card
        h1={isEditMode ? 'Edit Quotation' : 'Create Quotation'}
        buttonName="Back"
        navigation="-1"
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <Card title="Terms & Notes">
              <div className="flex flex-col gap-1">
                <Label>Notes</Label>
                <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} className="w-full rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3 py-2 text-sm outline-none resize-none text-[var(--vs-text-primary)] focus:border-indigo-500/60" />
              </div>
              <div className="flex flex-col gap-1">
                <Label>Terms &amp; Conditions</Label>
                <textarea value={terms} onChange={(e) => setTerms(e.target.value)} rows={3} className="w-full rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3 py-2 text-sm outline-none resize-none text-[var(--vs-text-primary)] focus:border-indigo-500/60" />
              </div>
            </Card>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <Card title="Quotation Details &amp; Summary" >
              <div className="flex flex-col gap-3.5">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <select value={invoiceType} onChange={(e) => setInvoiceType(e.target.value)} className="w-full rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-slate-900 px-3.5 py-2.5 text-sm outline-none text-[var(--vs-text-primary)]">
                ...
                <div className="flex items-center justify-between border-t border-gray-100 dark:border-white/5 pt-3">
                  ...
                  <input type="checkbox" checked={reverseCharge} onChange={(e) => setReverseCharge(e.target.checked)} className="h-4 w-4 rounded border-gray-300 text-indigo-600 outline-none cursor-pointer" />
                </div>

                <div className="flex items-center justify-between border-t border-gray-100 dark:border-white/5 pt-3">
                  <span className="text-sm font-semibold">Round Off Total</span>
                  <input type="checkbox" checked={roundOff} onChange={(e) => setRoundOff(e.target.checked)} className="h-4 w-4 rounded border-gray-300 text-indigo-600 outline-none cursor-pointer" />
                </div>
              </div>
              ...
              <div className="border-t border-gray-200 dark:border-white/10 pt-4 flex flex-col gap-2.5 text-sm">
                ...
                <div className="flex items-center justify-between text-[var(--vs-text-secondary)]">
                  ...
                  <div className="flex items-center gap-1.5 text-xs font-mono">
                    <input type="number" min="0" max="100" placeholder="%" value={overallDiscountPercent} onChange={(e) => setOverallDiscountPercent(Number(e.target.value))} className="w-12 rounded border border-gray-200 dark:border-white/10 bg-white dark:bg-slate-900 px-1 py-0.5 text-center text-xs outline-none text-[var(--vs-text-primary)]" />
                    <span>%</span>
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-2 border-t border-gray-200 dark:border-white/10 pt-4">
                <Button type="button" variant="primary" className="w-full flex items-center justify-center gap-2" onClick={() => handleSave('sent')} disabled={quotationLoading}>
                  {quotationLoading ? <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Send className="w-4 h-4" />}
                  {quotationLoading ? 'Saving...' : 'Save & Send'}
                </Button>
                <Button type="button" variant="outline" className="w-full flex items-center justify-center gap-2 border-dashed" onClick={() => handleSave('draft')} disabled={quotationLoading}>
                  {quotationLoading ? <span className="w-4 h-4 border-2 border-indigo-600/30 border-t-indigo-600 rounded-full animate-spin" /> : <Save className="w-4 h-4" />}
                  {quotationLoading ? 'Saving...' : 'Save Draft'}
                </Button>
              </div>
            </Card>
          </div>
        ...
        {/* ── Line Items ─────────────────────────────────────────────────────── */}
        <Card title="Line Items" rightNode={
            ...
            <table className="w-full text-sm border-collapse min-w-[700px]">
              <thead className="bg-gray-50/50 dark:bg-slate-800/50">
                <tr className="text-left text-xs font-bold uppercase tracking-wider text-[var(--vs-text-secondary)]">
                  <th className="py-3 px-3 border-r border-b border-gray-300 dark:border-white/20 min-w-[250px]">Product <span className="text-red-500">*</span></th>
                  <th className="py-3 px-3 border-r border-b border-gray-300 dark:border-white/20 w-[10%] text-center">Qty <span className="text-red-500">*</span></th>
                  <th className="py-3 px-3 border-r border-b border-gray-300 dark:border-white/20 w-[15%] text-center">Rate (₹) <span className="text-red-500">*</span></th>
                  <th className="py-3 px-3 border-r border-b border-gray-300 dark:border-white/20 w-[12%] text-center">Disc (%)</th>
                  <th className="py-3 px-3 border-r border-b border-gray-300 dark:border-white/20 w-[10%] text-center">GST (%)</th>
                  <th className="py-3 px-3 border-r border-b border-gray-300 dark:border-white/20 text-center w-[15%]">Total (₹)</th>
                  <th className="py-3 px-3 border-b border-gray-300 dark:border-white/20 w-[5%] text-center"></th>
                </tr>
              </thead>
              <tbody>
                {lineItems.map((item, idx) => {
                  const qty = Number(item.qty || 0), rate = Number(item.rate || 0);
                  const disc = Number(item.discountPercent || 0), gst = Number(item.taxPercent || 0);
                  ...
                  return (
                    <tr key={idx} className={`${!isLast ? 'border-b border-gray-300 dark:border-white/20' : ''} align-middle hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition-colors`}>
                      ...
                      <td className="py-2.5 px-3 border-r border-gray-300 dark:border-white/20 align-top">
                        <Input type="number" min="0" step="0.01" value={item.rate} onChange={(e) => handleLineItemChange(idx, 'rate', Number(e.target.value))} className={`text-center !h-9 !rounded-lg ${isSubmitted && Number(item.rate) < 0 ? '!border-red-500' : ''}`} />
                      </td>
                      <td className="py-2.5 px-3 border-r border-gray-300 dark:border-white/20 align-top">
                        <Input type="number" min="0" max="100" value={item.discountPercent} onChange={(e) => handleLineItemChange(idx, 'discountPercent', Number(e.target.value))} className="text-center !h-9 !rounded-lg" />
                      </td>
                      ...
                      <td className="py-2.5 px-3 text-center font-semibold align-top pt-4 border-r border-gray-300 dark:border-white/20 text-indigo-600 dark:text-indigo-400">
                        ₹{lineTotal.toFixed(2)}
                      </td>
              ...
              return (
                <div key={idx} className="border border-gray-200 dark:border-white/10 rounded-2xl p-4 bg-gray-50/50 dark:bg-white/[0.01] flex flex-col gap-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="flex flex-col gap-1"><Label>Rate (₹) *</Label><Input type="number" min="0" step="0.01" value={item.rate} onChange={(e) => handleLineItemChange(idx, 'rate', Number(e.target.value))} /></div>
                    <div className="flex flex-col gap-1"><Label>Discount (%)</Label><Input type="number" min="0" max="100" value={item.discountPercent} onChange={(e) => handleLineItemChange(idx, 'discountPercent', Number(e.target.value))} /></div>
                    <div className="flex flex-col gap-1"><Label>GST</Label><div className="w-full rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-slate-800/50 px-3 py-2.5 text-sm text-center font-medium text-[var(--vs-text-primary)]">{item.isGstApplicable ? `${item.taxPercent}%` : '-'}</div></div>
                  </div>
                  <div className="flex justify-between items-center border-t border-gray-200 dark:border-white/5 pt-2 text-sm font-semibold">
          ...
          {isSubmitted && lineItems.some((item) => !item.productId || Number(item.qty) <= 0 || Number(item.rate) < 0) && (
            <div className="text-red-500 text-sm mt-2 font-medium">Please fill out all required line items with valid products, quantities, and rates.</div>
          )}
        </Card>
      </Card>
    </div>
