...
import {
  Plus, Search, Eye, Edit2, Send, Check, X, Trash2, MoreVertical,
  ArrowRightCircle, Receipt, Truck, FileText, CheckCircle2, RefreshCw, XCircle
} from 'lucide-react';
...
const STATUS_CONFIG = {
  ...
  issued: { label: 'Issued', textCls: 'text-blue-600 dark:text-blue-400', icon: <Send className="w-4 h-4 text-blue-500" /> },
  delivered: { label: 'Delivered', textCls: 'text-emerald-600 dark:text-emerald-400', icon: <CheckCircle2 className="w-4 h-4 text-emerald-500" /> },
  converted: { label: 'Converted', textCls: 'text-violet-600 dark:text-violet-400', icon: <RefreshCw className="w-4 h-4 text-violet-500" /> },
  cancelled: { label: 'Cancelled', textCls: 'text-rose-600 dark:text-rose-400', icon: <XCircle className="w-4 h-4 text-rose-500" /> },
};
...
export default function DeliveryChallanList() {
  ...
  const { deliveryChallans, summary, loading, meta, toast: reduxToast } = useSelector((state) => state.deliveryChallan);
  ...
  const fetchChallans = () => {
    ...
    dispatch(getDeliveryChallans(params));
  };

  useEffect(() => { fetchChallans(); }, [dispatch, currentPage, pageSize, selectedStatus]);
  ...
  const handleViewClick = (item) => {
    ...
    navigate(`/sales/delivery-challan/view/${encId}`);
  };

  const handleEditClick = (item) => {
    if (item.status !== 'draft') { showToast('Only draft challans can be edited.', 'warning'); return; }
    const encId = encodeURIComponent(encryptData(item._id));
    navigate(`/sales/delivery-challan/edit/${encId}`);
  };

  const handleDeleteClick = (item) => {
    if (item.status !== 'draft') { showToast('Only draft challans can be deleted.', 'warning'); return; }
    setDeleteModal({ isOpen: true, id: item._id });
  };

  const handleConfirmDelete = () => {
    if (deleteModal.id) { dispatch(deleteDeliveryChallan(deleteModal.id)); }
  ...
  const handleConfirmAction = () => {
    ...
    if (type === 'issue') {
      dispatch(updateDeliveryChallanStatus({ id, status: 'issued' }));
    } else if (type === 'deliver') {
      dispatch(updateDeliveryChallanStatus({ id, status: 'delivered' }));
    } else if (type === 'cancel') {
      dispatch(updateDeliveryChallanStatus({ id, status: 'cancelled' }));
  ...
  const headers = [
    ...
    },
    {
      ...
      sortable: true,
      value: 'checked',
      render: (item) => <span>{formatDateWithTiming(item.challanDate)}</span>,
    },
    {
      label: 'Customer',
      key: 'customer',
    ...
    },
    {
      ...
      sortable: true,
      value: 'checked',
      render: (item) => <span className="font-mono text-sm">{item.vehicleNumber || '—'}</span>,
    },
    {
      ...
      sortable: true,
      value: 'checked',
      render: (item) => <span>{item.transportMode || '—'}</span>,
    },
    {
      label: 'Status',
      key: 'status',
  ...
  const confirmConfig = {
    issue: { title: 'Issue Delivery Challan?', message: 'This will mark the challan as issued. You cannot edit it after issuing.', label: 'Issue', variant: 'primary', icon: <Send className="w-6 h-6 text-blue-500" /> },
    ...
    invoice: { title: 'Convert to Invoice?', message: 'A new sales invoice will be created from this delivery challan.', label: 'Convert', variant: 'primary', icon: <Receipt className="w-6 h-6 text-emerald-500" /> },
  };
    ...
    <div>
      ...
      >
        <div className="pb-2">
          <TableInfoCard
            stats={Object.entries(STATUS_CONFIG).map(([key, cfg]) => {
              ...
              return {
                label: cfg.label,
                value: metric.count || deliveryChallans.filter((c) => c.status === key).length,
        ...
        <Table
          ...
          onSearchClick={() => { setCurrentPage(1); fetchChallans(); }}
          onSearchClear={handleClearSearch}
          actions={
            <div className="flex items-center gap-2">
              <Button onClick={() => navigate('/sales/delivery-challan/create')}>New Delivery Challan</Button>
            </div>
          }
          emptyMessage="No delivery challans found. Click 'New Delivery Challan' to create one."
      ...
      <DeleteModal
        ...
        message="Are you sure you want to delete this draft delivery challan? This cannot be undone."
      />
          ...
          >
            ...
            {dropdownItem.status === 'draft' && (
              <button onClick={() => { setActiveDropdownId(null); openConfirm('issue', dropdownItem._id); }}
                ...
                <Send className="w-4 h-4 mr-2.5" /> Issue Challan
              </button>
            )}

            {/* Deliver (draft/issued) */}
            {(dropdownItem.status === 'draft' || dropdownItem.status === 'issued') && (
              <button onClick={() => { setActiveDropdownId(null); openConfirm('deliver', dropdownItem._id); }}
                ...
                <Check className="w-4 h-4 mr-2.5" /> Mark Delivered
              </button>
            )}

            {/* Convert action (issued or delivered) */}
            {(dropdownItem.status === 'issued' || dropdownItem.status === 'delivered') && (
              <button onClick={() => { setActiveDropdownId(null); openConfirm('invoice', dropdownItem._id); }}
                ...
                <ArrowRightCircle className="w-4 h-4 mr-2.5" /> Convert to Invoice
              </button>
            )}

            {/* Cancel (issued or delivered) */}
            {(dropdownItem.status === 'issued' || dropdownItem.status === 'delivered') && (
              <button onClick={() => { setActiveDropdownId(null); openConfirm('cancel', dropdownItem._id); }}
                ...
                <X className="w-4 h-4 mr-2.5" /> Cancel Challan
              </button>
            )}
